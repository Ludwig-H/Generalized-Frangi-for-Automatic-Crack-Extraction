...
from frangi_fusion.clustering import hdbscan_from_sparse
...
def process_one(struct, idx, sigmas, beta, c, ctheta, R, K, expZ, crackseg_dir=None):
    ...
    base = dat["arrays"].get("intensity", None)
    rng  = dat["arrays"].get("range", None)
    fused= dat["arrays"].get("fused", None)
    gt   = dat["arrays"].get("label", None)
    if base is None or gt is None:
        return None
    Hmods = {}
    Hmods["intensity"] = compute_hessians_per_scale(to_gray(base), sigmas)
    if rng is not None:
        Hmods["range"] = compute_hessians_per_scale(to_gray(rng), sigmas)
    if fused is not None:
        Hmods["fused"] = compute_hessians_per_scale(to_gray(fused), sigmas)
    w = {k:1.0 for k in Hmods.keys()}
    fused_H = fuse_hessians_per_scale(Hmods, w)
    coords, neigh, S = build_frangi_similarity_graph(fused_H, beta, c, ctheta, R)
    ...
    if crackseg_dir is not None and os.path.isdir(crackseg_dir):
        import glob, re, imageio
        key = re.findall(r"\\d+", os.path.basename(dat["paths"]["label"]))
        key = key[-1] if key else os.path.basename(dat["paths"]["label"])
        cands = glob.glob(os.path.join(crackseg_dir, f"*{key}*"))
        if cands:
            pred = imageio.imread(cands[0])
            if pred.ndim==3: pred = pred[...,0]
            sk_ck = skeletonize_lee(pred>0)
            sk_ck = thicken(sk_ck, pixels=6)
...
def main():
    ...
    ap.add_argument("--find-root", required=True, help="Path to FIND dataset root (unzipped).")
    ap.add_argument("--cracksegdiff-dir", default=None, help="Directory with CrackSegDiff outputs (optional).")
