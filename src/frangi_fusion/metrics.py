...
def skeletonize_lee(binary_mask: np.ndarray) -> np.ndarray:
    m = (binary_mask > 0.5).astype(np.uint8)
    sk = skeletonize(m>0)
...
def thicken(skel: np.ndarray, pixels: int = 6) -> np.ndarray:
    ...
    selem = disk(int(pixels))
    thick = dilation(skel>0, selem)
...
def wasserstein_distance_skeletons(A: np.ndarray, B: np.ndarray, max_samples: int = 2000) -> float:
    import ot
    Ay, Ax = np.nonzero(A>0)
    By, Bx = np.nonzero(B>0)
    ...
    if A_pts.shape[0] > max_samples:
        idx = np.random.choice(A_pts.shape[0], size=max_samples, replace=False)
        A_pts = A_pts[idx]
    if B_pts.shape[0] > max_samples:
        idx = np.random.choice(B_pts.shape[0], size=max_samples, replace=False)
        B_pts = B_pts[idx]
